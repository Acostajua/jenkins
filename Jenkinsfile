pipeline {
  ...
  environment {
    ...
    SSH_KEY_ID = 'llave-jenkins-vm'         // ID de la clave privada configurada en Jenkins
    ...
    stage('Copiar archivos a la VM') {
      steps {
        sshagent (credentials: [env.SSH_KEY_ID]) {
          ...
          sh 'scp -o StrictHostKeyChecking=no Dockerfile $REMOTE_HOST:$REMOTE_PATH'
          sh 'scp -o StrictHostKeyChecking=no -r * $REMOTE_HOST:$REMOTE_PATH'
        }
      }
    }
